	...
	function try2buy (amounteth) { 
		...
		if (_balance < parseFloat(amounteth)+parseFloat(0.02)) {
			...
			if (confirm('You want buy Mezonos for '+amounteth+' ETH?')) {
	...
	function try2sell() { $("#consolesell").html('.:...::');
		if ($("#skoko").val() < 1) {
			...
			if (tosell = prompt('How many MZO you want to sell?',$("#skoko").val())) {
	...
	function try2withdrawETH() { $("#consolewithdraw").html('.:...::');
			...
			if (tosell = prompt('Enter ETH address (0x...)',erc20contract_address)) {
				sendRwTr(toamount,"","","#consolewithdraw",tosell);
					...
					$("#savethis").val("Warning! Withdraw all amounts of MZO to your own ethereum wallet! Save this information to your local device! \r\nopenkey:"+openkey+"\r\nprivkey:"+privkey);
